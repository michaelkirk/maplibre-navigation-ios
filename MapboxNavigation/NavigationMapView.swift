...
open class NavigationMapView: MLNMapView, UIGestureRecognizerDelegate {
    ...
    fileprivate func commonInit() {
        ...
        self.mapTapGesture.requireFailure(of: gestures)
        self.mapTapGesture.delegate = self
        addGestureRecognizer(self.mapTapGesture)
    }
    ...
    @objc func progressDidChange(_ notification: Notification) {
        ...
        if let location = userLocationForCourseTracking {
            ...
            if !cameraUpdated {
                ...
                // the user location puck in the lower portion of the visible map.
                let contentFrame = bounds.inset(by: safeAreaInsets)
                let edgePadding = UIEdgeInsets(top: contentFrame.height * 0.5, left: 0, bottom: 0, right: 0)
